...
class CsvProcessor:
    ...
    def process_df(self, partition_df, csv_date):
        def persist_row(series_obj, date):
            values = [date, str(series_obj[self.USI_CSV_GROUP_ID]), str(series_obj[self.USI_CSV_KEYWORD_ID]), self.format_date(str(
                series_obj[self.USI_CSV_TIMESTAMP])), series_obj[self.USI_CSV_IMPRESSIONS], series_obj[self.USI_CSV_CLICKS], series_obj[self.USI_CSV_KEYWORD]]
