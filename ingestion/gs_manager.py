...
class GoogleStorageManager:
    ...
    def get_filenames(self, path_in_bucket=''):
        csv_full_paths = self.fs.ls(
            self.USI_GOOGLE_CLOUD_BUCKET + '/' + path_in_bucket)
        csv_paths_in_bucket = []
        for csv_full_path in csv_full_paths:
            csv_paths_in_bucket.append(csv_full_path.replace(
                self.USI_GOOGLE_CLOUD_BUCKET + '/', '', 1))
    ...
    def move_to_processed(self, path_in_bucket):
        ...
        path_to = self.USI_GOOGLE_CLOUD_BUCKET + '/' + \
            self.USI_GOOGLE_CLOUD_PROCESSED + "/" + \
            os.path.basename(path_in_bucket)
        self.fs.mv(path_from, path_to)

    def get_df(self, csv_path_in_bucket):
        df = dd.read_csv('gcs://' + self.USI_GOOGLE_CLOUD_BUCKET +
                         '/' + csv_path_in_bucket, sep="\t")
        return df
